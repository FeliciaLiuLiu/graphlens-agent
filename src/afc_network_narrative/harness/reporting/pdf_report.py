...
def build_report_lines(
    ...
    *,
    ...
    sar_red_flag_lines = build_sar_red_flag_lines(payload.get("sar_red_flags", {}), report_policy)
    lines.extend(render_section(section_titles["sar_red_flags"], sar_red_flag_lines, bullet=True))
...
def build_sar_red_flag_lines(sar_red_flags: dict[str, Any], report_policy: dict[str, Any]) -> list[str]:
    if not sar_red_flags:
        return ["No SAR/STR red flag review signals were generated by the configured graph rules."]

    template = report_policy["sar_red_flag_line_template"]
    lines = [
        sar_red_flags.get(
            "summary",
            "This system identifies SAR/STR red flag review signals only and does not make filing decisions.",
        )
    ]
    for item in sar_red_flags.get("matched_review_signals", []):
        lines.append(
            template.format(
                name=item["name"],
    ...
    if not sar_red_flags.get("matched_review_signals"):
        lines.append("No graph-supported SAR/STR red flag review signals matched.")
    return lines
